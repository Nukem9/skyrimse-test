...
bool BSShaderResourceManager_CK::FindIntersectionsTriShapeFastPathEx(NiPoint3& kOrigin, const NiPoint3& kDir, NiPick& kPick, class BSTriShape* pkTriShape) {
	//
	// This function is for the new version of CK. 
	// It is no different from the old one, except that there is no more offset 0x20 to get the desired value.
	//
	...
	if (indices && vertexData[0])
	{
		...
		for (uint32_t i = 0; i < (pkTriShape->m_TriangleCount * 3u); i += 3)
		{
			...
			if (NiCollisionUtils::IntersectTriangle(kModelOrigin, kModelDir, v[0], v[1], v[2], kPick.GetFrontOnly(), intersect, lineParam, triParam1, triParam2))
			{
				auto record = ((NiPick::Record * (__fastcall*)(uintptr_t, BSTriShape*))OFFSET(0x26D7910, 1530))((uintptr_t)&kPick, pkTriShape);
				intersectionFound = true;
